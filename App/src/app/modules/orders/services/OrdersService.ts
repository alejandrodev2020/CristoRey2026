...
export class OrdersService extends BaseService {
  ...
  async getOrdersSale(queryString: string): Promise<any> {
    try {
      return await this.get(`api/quotation/list${queryString}`);
    } catch (error) {
  ...
  async getOrderById(id:number): Promise<any> {
    try {
      return await this.get(`api/quotation/${id}`);
    } catch (error) {
  ...
  async store(data: any): Promise<any> {
    try {
        return await this.post('api/distribution/order', data);
    } catch (error) {
