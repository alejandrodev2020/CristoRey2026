...
import { View, Text, FlatList, ActivityIndicator, Image, TouchableOpacity, Dimensions, StatusBar} from 'react-native';
...
// import { styles as style } from './ordersStoreScreenStyle';
import { styles as style } from './OrdersStoreScreenStyle';
...
type Props = DrawerScreenProps<DrawerParamList, 'OrdersStore'>;
  const { width: screenWidth, height: screenHeight } = Dimensions.get('window');
export default function OrdersStoreScreen({navigation}: Props) {
    ...
    const init = async () => {
      ...
      if (cachedList && cachedList.length > 0) {
        ...
        if (cachedList.length === 1) {
          ...
          navigation.navigate('SelectClientOrdersStore', { warehouseId: cachedList[0].id });
        }

        return;
      }

      // Si no hay data cacheada, hacemos fetch
      setPage(0);
      setHasMore(true);
      setListWarehouse([]);
      await loadItems();
...
const loadItems = async () => {
  ...
  try {
    ...
    const activeWarehouses = (response || []).filter((item : any) => item.isActive);
    ...
    if (activeWarehouses.length === 1 && page === 0) {
      ...
      navigation.navigate('SelectClientOrdersStore', { warehouseId: warehouse.id });
      return;
    }
  ...
  return (
    <>
     ...
     <WaveDivider />
        <View style={{top: 48, paddingHorizontal:20}}>
          <View style={style.row}>
            ...
            <Text style={style.title}>
             Listado de Sucursales - Pedidos
            </Text>
          </View>
        </View>

      <FlatList
        style={{flex: 1,marginTop:50, paddingHorizontal:15}}
        data={listWarehouse}
            ...
            <TouchableOpacity
              ...
              <LinearGradient
                ...
                <View style={style.card}>
                  <Text style={style.cardTitle}>Vista previa de Almacén</Text>
                ...
                <View style={style.footer}>
                  <View style={style.footerSection}>
                    ...
                    <Text style={{ fontFamily: 'Poppins-Regular', fontSize:12}}>{item.description}</Text>
                  </View>
