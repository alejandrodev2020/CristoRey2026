...
import Images from '../../../../assets/images/images'; // Asegúrate de tener una imagen de micrófono aquí
...
export default function RequestPermissionMicrophone({ navigation }: Props) {
    ...
    try {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.RECORD_AUDIO,
        {
          ...
          message: 'La app necesita acceder al micrófono para grabar audio.',
      ...
      if (granted === PermissionsAndroid.RESULTS.GRANTED) {
        ...
        Alert.alert('Permiso concedido', 'Ya puedes usar el micrófono.');
      } else {
        setPermissionStatus('Denegado');
        Alert.alert('Permiso denegado', 'No podrás grabar audio sin este permiso.');
      }
    } catch (error) {
      console.error('Error al solicitar permiso de micrófono:', error);
      Alert.alert('Error', 'Ocurrió un error al solicitar el permiso.');
    }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      {/* Imagen de fondo (usa una relacionada al micrófono) */}
      <Image
        source={Images.bacground.microphone} // ⚠️ Asegúrate de tener esta imagen
