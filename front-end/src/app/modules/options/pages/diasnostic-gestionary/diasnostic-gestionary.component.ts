...
@Component({
  selector: 'app-diasnostic-gestionary',
  templateUrl: './diasnostic-gestionary.component.html',
  styleUrls: ['./diasnostic-gestionary.component.scss']
})
export class DiasnosticGestionaryComponent implements AfterViewInit {
  form: FormGroup;
  ...
  ngOnInit(): void {
    this.service.getListDiasnosticByOptionId(this.id).subscribe((ele: any) => {
  ...
  createNew() {
    this.router.navigate([`options/${this.id}/diasnostic/store`]);
  }
  addMassive() {
  ...
  update(data: any) {
    this.router.navigate([`options/${this.id}/diasnostic/store/${data.id}`]);
  }
