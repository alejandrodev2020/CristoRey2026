...
@Component({
  selector: 'filter-petty-cash-gestionary',
  templateUrl: './filter-petty-cash-gestionary.component.html',
  styleUrls: ['./filter-petty-cash-gestionary.component.scss']
})
export class FilterPettyCashGestionaryComponent {
  ...
  @Output('emitQuery') change: EventEmitter<any> = new EventEmitter<any>();
  ...
  save() {
    ...
    if (!allNull) {
      ...
      if (filter.statusId != null) {
        queryString += ((queryString.length > 1) ? '&' : '') + 'StatusId=' + filter.statusId;
      }
