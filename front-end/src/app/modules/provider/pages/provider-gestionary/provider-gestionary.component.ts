...
@Component({
  selector: 'app-provider-gestionary',
  templateUrl: './provider-gestionary.component.html',
  styleUrls: ['./provider-gestionary.component.scss']
})
export class ProviderGestionaryComponent implements AfterViewInit{
  ...
  constructor(private service: ProviderService,private fb: FormBuilder,
  ...
  actions: any[] = [];
  ngOnInit(): void {
    this.service.getListProvider().subscribe((ele: any) => {
  ...
  createNew() {
    this.router.navigate(['provider/provider/store']);
  }

  update(data: any) {
    this.router.navigate(['provider/provider/store/' + data.id]);
  }
