...
constructor(private dialogRef: MatDialogRef<any>,
  ...
  async setListUnitmeasurenmetOrigin(data: any) {
    try {
        const resp: any = await firstValueFrom(this.service.getAllProductEquivalence(data?.product?.id));
        this.listUnitMeasurementOrigin = resp;
        await this.setListEquivalences();
        await this.setValuesDefault(this.product);
    } catch (error) {
        console.error('Error fetching product equivalence:', error);
    }
  }
 ...
 setValuesDefault(product : WarehouseProduct) {
      ...
      let ae = product.unitMeasurement.id;
      let a = this.listUnitMeasurement.find(obj => obj.unitMeasurementId === ae);
      if(a.suggestedPriceSale && a !== null){
         this.form.get('suggestedPrice').setValue(a.suggestedPriceSale);
         this.form.get('amountOrigin').setValue(a.suggestedPriceSale);
      }
      else{
  ...
  async setListEquivalences() {
    try {
        const resp: any = await firstValueFrom(this.service.getListWarehouseProductEquivalence(this.product?.id));
        this.listUnitMeasurement = resp;
        let a = this.product.unitMeasurement.id;
        let b = this.checkUnitMeasurement?.id;
        this.listUnitMeasurement.forEach(obj => {
            if (obj.unitMeasurementId === a || obj.unitMeasurementId === b ) {
                obj.check = true;
