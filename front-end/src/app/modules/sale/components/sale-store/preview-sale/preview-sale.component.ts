...
import { SaleCreditStoreComponent } from '../sale-credit-store/sale-credit-store.component';
import { OverviewQrCodeComponent } from '../../overview-qr-code/overview-qr-code.component';
...
import { FormMixPaymentComponent } from '../form-mix-payment/form-mix-payment.component';
...
@Component({
  selector: 'app-preview-sale',
  templateUrl: './preview-sale.component.html',
  styleUrls: ['./preview-sale.component.scss']
})
export class PreviewSaleComponent implements AfterViewInit  {
  form: FormGroup;
  options: any[] = [];
  myDate = new Date();
  ...
  saveShopping() {
    ...
    if (this.isValidForm()) {
      ...
      if(data.saleTypeId !==  SaleTypes.COTIZACION)
        {
            this.service.store(data).subscribe((resp : number) => {
                this.close();
                this.buttonDisabled = true
                if(resp !== null){
